...
from my_custom_widgets import QuestionButton, CustomFrame, CustomLabel
...
class TestYourKnowledge(tk.Frame):
    ...
    def populate_page(self):
        ...
        """
        if self.master.embeddedPages.frames["TestConfigurer"].options.entry.get() != '':
            try:
                self.buttonNumber = int(self.master.embeddedPages.frames["TestConfigurer"].options.entry.get())
            except ValueError:
                self.buttonNumber = 3
        else:
            self.buttonNumber = 3
    ...
    def create_progressbar(self, row=0, column=0):
        ...
        if self.master.embeddedPages.frames["TestConfigurer"].questions.entry.get() == '':
            maximum = 10
        else:
            try:
                maximum = int(self.master.embeddedPages.frames["TestConfigurer"].questions.entry.get())
            except ValueError:
                maximum = 10
