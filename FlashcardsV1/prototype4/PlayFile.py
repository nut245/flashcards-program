...
class Play(ctk.CTkFrame):
    ...
    def create_custom_button(self, text, column, row):
        if text == 'shuffle':
            ...
            return CustomButton.RestartButton(self.buttonFrame, controller=self, main=self.controller, text=text).grid(column=column, row=row, rowspan=2, padx=5, pady=5, sticky='news')
